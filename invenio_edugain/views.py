...
def disco_feed() -> list:
    ...
    discoverable_query = db.select(IdPData).where(
        IdPData.discoverable == true(),
        IdPData.enabled == true(),
    )
    discoverable_idp_ids: set[str] = {
        idp_data.id for idp_data in db.session.scalars(discoverable_query)
    }
