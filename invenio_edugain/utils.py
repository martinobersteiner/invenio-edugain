...
class AuthnInfo:
    ...
    id_by_method: dict[str, str | None]  # NOTE: hashed, preferred methods first
    ...
    @classmethod
    def from_saml_xml(cls, saml_xml_response: str, next_: str | None = None) -> Self:
        ...
        id_by_method["pairwise-id"] = ava["id"][0]  # TODO: remove
...
def create_user(authn_info: AuthnInfo) -> User:
    """Create user and link it with (external_method, external_id).
    ...
    if form.validate():
        ...
        confirmed_at = datetime.now(timezone.utc)  # TODO: make this configurable
        data = {
            **form.to_dict(),
        ...
        if not data["password"]:
            user.password = None  # TODO: this is from invenio-saml, does user.password=None prevent local login?
        current_app.extensions["security"].datastore.commit()
    else:
        msg = "form failed to validate when trying to create a user"
        raise AuthnResponseError(msg)

    if UserIdentity.get_user(method=method, external_id=external_id):
        # already linked
        pass
    else:
